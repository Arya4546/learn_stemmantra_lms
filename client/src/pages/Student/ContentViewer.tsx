...
export function ContentViewer() {
  ...
  const [contentData, setContentData] = useState<{ type: string; title: string } | null>(null);
  ...
  useEffect(() => {
    const fetchToken = async () => {
      try {
        ...
        const { token, type, title } = response.data.data;
        
        setContentData({ type, title });
      ...
      <div className="bg-white p-6 rounded-xl border border-border shadow-sm">
        ...
        <div className="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-200 text-sm text-amber-800 flex gap-3 items-start">
          ...
          <p className="font-medium">
            This {contentData.type.toLowerCase()} is protected by a single-use access token and is bound to your account ({user?.email}). 
            Screen recording, downloading, or unauthorized sharing is strictly prohibited and tracked.
          </p>
        </div>
      </div>
    </div>
